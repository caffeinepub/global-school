...
export default function ReportsPage() {
  ...
  const handleExport = async (type: "daily" | "monthly" | "yearly") => {
    setExporting(true);
    try {
      let rows: ReturnType<typeof getAllPaymentRows> = [];
      let filename = "";
      const today = new Date().toLocaleDateString("en-IN").replace(/\//g, "-");

      if (type === "daily") {
        rows = getDailyPaymentRows(
          students,
          new Date().toLocaleDateString("en-IN"),
        );
        // Export all payments for all days
        rows = dailyReport.periods.flatMap((p) =>
          getDailyPaymentRows(students, p.label),
        );
        filename = `Daily_Report_${today}`;
      } else if (type === "monthly") {
        rows = monthlyReport.periods.flatMap((p) =>
          getMonthlyPaymentRows(students, p.label),
        );
        filename = `Monthly_Report_${today}`;
      } else {
        rows = yearlyReport.periods.flatMap((p) =>
          getYearlyPaymentRows(students, p.label),
        );
        filename = `Yearly_Report_${today}`;
      }
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                disabled={exporting}
                data-ocid={`reports.${type}_export.button`}
    ...
    <div className="space-y-6">
      ...
      >
        <TabsList className="grid w-full max-w-md grid-cols-3">
          ...
          <TabsTrigger value="yearly" data-ocid="reports.yearly.tab">
            Yearly
          </TabsTrigger>
        </TabsList>
